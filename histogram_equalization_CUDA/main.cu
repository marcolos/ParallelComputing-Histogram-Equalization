...
Mat print_image(unsigned char *image,int height,int width){
...
int main(){
	string folder_path = "/home/giuliocalamai/eclipse-workspace/histogram_equalization_CUDA/img/";
	...
	int host_cumulative_dist[256];				//cpu cumulative histogram
	...
	int block_size = 256;													// number of thread per block
	int grid_size = (width * height + (block_size - 1))/block_size;     	// number of block of the grid
	...
	//compute cumulative dist. in cpu
	...
	//call third kernel
	YUV2RGB<<<grid_size, block_size>>>(device_image, device_cumulative_dist, device_histogram, device_equalized, width, height);
	...
	// Non si riesce a convertire il puntatore in un tipo Mat. Per fare questo si chiama la funzione print_image
	//Mat final_image = Mat(Size(width,height),CV_8UC3, host_image);
	//Mat final_image(width,height,CV_8UC3, host_image);

	Mat final_image = print_image(host_image,height,width);
	//imshow("Equalized",final_image);

	string save_folder_path = "/home/giuliocalamai/eclipse-workspace/histogram_equalization_CUDA/img_after_eq/";
